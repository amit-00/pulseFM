    ...
    <div className="flex flex-col gap-2">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-baseline gap-2">
          <span className="text-[10px] uppercase tracking-wide text-stone-500">{timeLabel}</span>
          <span
            className={cn(
              "text-xs font-mono font-semibold tabular-nums",
              voteClosed ? "text-stone-400" : "text-stone-300",
            )}
          >
            {formattedTime}
          </span>
      ...
      {voteClosed ? (
        <div className="rounded-lg border border-stone-700/30 bg-stone-900/20 p-3">
          <p className="text-[11px] uppercase tracking-wide text-stone-500">Winning Option</p>
          {isWinnerLoading ? (
            <div className="mt-2 space-y-2">
              <div className="h-4 w-32 rounded bg-stone-700/50 animate-pulse" />
              <div className="h-3 w-44 rounded bg-stone-700/40 animate-pulse" />
            </div>
          ) : winnerLabel ? (
            ...
            <p className="mt-1 text-sm font-medium text-stone-300">No winner declared</p>
          )}
        </div>
      ) : (
