...
resource "google_secret_manager_secret" "cdn_signed_cookie_key" {
  secret_id = "cdn-signed-cookie-key"
  replication {
    auto {}
  }
}

resource "google_secret_manager_secret_version" "cdn_signed_cookie_key" {
  secret      = google_secret_manager_secret.cdn_signed_cookie_key.id
  secret_data = var.cdn_signed_cookie_key_value
}
...
resource "google_secret_manager_secret_iam_member" "cdn_signed_cookie_key_accessor" {
  secret_id = google_secret_manager_secret.cdn_signed_cookie_key.id
  role      = "roles/secretmanager.secretAccessor"
  member    = "serviceAccount:${google_service_account.nextjs_server.email}"
}
